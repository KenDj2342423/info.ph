...
// ==========================================
// LIVE WEATHER (9 Cities) - CORS BYPASS
// ==========================================
  ...
  try {
    const weatherData = await Promise.all(
      WEATHER_CITIES.map(async city => {
        try {
          const url = `https://api.openweathermap.org/data/2.5/weather?lat=${city.lat}&lon=${city.lon}&units=metric&appid=${WEATHER_API_KEY}`;
...
// ==========================================
// INTERACTIVE CALENDAR WITH MANUAL DATE PICKER
// ==========================================
...
// ==========================================
// MANUAL DATE PICKER MODAL
// ==========================================
function initializeDatePicker() {
  const modal = document.getElementById('datePickerModal');
  const manualPickerBtn = document.getElementById('manualDatePicker');
  ...
  if (!modal || !manualPickerBtn) return;
  ...
  function populateDays() {
    ...
    // Restore previous day if valid
    if (currentDay <= daysInMonth) {
      daySelect.value = currentDay;
    }
  }

  // Set current date in selects
  ...
  // Open modal
  manualPickerBtn.addEventListener('click', () => {
    setCurrentDate();
  ...
  applyBtn.addEventListener('click', () => {
    ...
    console.log('Jumped to:', currentDate.toLocaleDateString());
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log('📅 Calendar: Interactive with manual date picker');
  console.log('☰ Sidebar: Always accessible');
  console.log('🛍️ Shopping: Complete section added');
});